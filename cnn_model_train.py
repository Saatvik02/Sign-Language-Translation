...
def get_image_size():
	img_path = 'gestures/101/1.jpg'  
	# Check if the file exists
	if not os.path.isfile(img_path):
			raise FileNotFoundError(f"Image at path '{img_path}' does not exist.")
	
	# Check read permissions
	if not os.access(img_path, os.R_OK):
			raise PermissionError(f"No read permission for image at path '{img_path}'.")

	# Attempt to read the image
	img = cv2.imread(img_path, 0)
	if img is None:
			raise ValueError(f"Image at path '{img_path}' could not be read. It may be corrupted or in an unsupported format.")
